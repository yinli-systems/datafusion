...
### New fields for automatic dictionary aggregation

The opt-in automatic dictionary aggregation path adds these fields:

- `OptimizerOptions::enable_dictionary_aggregation: bool` (default `false`)
- `FileMeta::physical_statistics: Option<PhysicalFileStatistics>`
- `CachedFileMetadata::physical_statistics: Option<PhysicalFileStatistics>`

Downstream crates that construct any of these public structs with exhaustive
struct literals must add the corresponding field. Use `false` for
`enable_dictionary_aggregation` and `None` for either `physical_statistics`
field. Code that uses `ConfigOptions::default()`, `FileMeta::new(...)`, or
`CachedFileMetadata::new(...)` requires no change.
